...
public class SSTools : MonoBehaviour
{
    ...
    public static void ShowMessage(string msg, SSTools.Position position, SSTools.Timee time)
    {
        if (!clone)
        {
    ...
    private static void RemoveClone(GameObject clone, Timee time)
    {
        


        if (time == Timee.oneSecond)
        {
            
            Destroy(clone.gameObject, 1f);
            
        }
        else if (time == Timee.twoSecond)
        {
